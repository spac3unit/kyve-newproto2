...
import {
    getBalanceByAddress,
    getStakersList,
    getPoolById, sleep, getDelegator, getDelegationPoolData,
} from "./helpers/utils";
...
export const delegation = () => {
    ...
    test("No Self delegation", async () => {
        ...
        const receiptAlice = await (await alice.delegate(0, ADDRESS_ALICE, amount)).transactionBroadcast
        const receiptBob = await (await bob.delegate(0, ADDRESS_BOB, amount)).transactionBroadcast
        const receiptCharlie = await (await charlie.delegate(0, ADDRESS_CHARLIE, amount)).transactionBroadcast
    ...
    test("Delegate into Alice", async () => {
        ...
        const receiptBob = await (await bob.delegate(0,ADDRESS_ALICE, bobDelegation)).transactionBroadcast;
        expect(receiptBob.code).toEqual(0);

        // Delegate 300 $KYVE from Charlie into Alice
        const receiptCharlie = await (await charlie.delegate(0,ADDRESS_ALICE, charlieDelegation)).transactionBroadcast;
        expect(receiptCharlie.code).toEqual(0);

        // Test queries
        const delegatorBob = await getDelegator(ADDRESS_BOB, ADDRESS_ALICE, 0);
        expect(delegatorBob.delegation_amount).toEqual(bobDelegation.toString());

        const delegatorCharlie = await getDelegator(ADDRESS_CHARLIE, ADDRESS_ALICE, 0);
        expect(delegatorCharlie.delegation_amount).toEqual(charlieDelegation.toString());
        ...
        expect(preBalanceCharlie.minus(charlieDelegation)).toEqual(postBalanceCharlie);
        ...
        expect(delegationPoolData.total_delegation).toEqual(bobDelegation.plus(charlieDelegation).toString());
    ...
    test("Undelegate everything from Alice", async () => {
        ...
        // Delegate 100 $KYVE from Bob into Alice
        const receiptBob = await (await bob.undelegate(0,ADDRESS_ALICE, bobDelegation)).transactionBroadcast;
        expect(receiptBob.code).toEqual(0);

        // Delegate 300 $KYVE from Charlie into Alice
        const receiptCharlie = await (await charlie.undelegate(0,ADDRESS_ALICE, charlieDelegation)).transactionBroadcast;
        expect(receiptCharlie.code).toEqual(0);

        // Test queries
        let delegatorBob = await getDelegator(ADDRESS_BOB, ADDRESS_ALICE, 0);
        expect(delegatorBob).toEqual(undefined);

        let delegatorCharlie = await getDelegator(ADDRESS_CHARLIE, ADDRESS_ALICE, 0);
        expect(delegatorCharlie).toEqual(undefined);

        await sleep(UNSTAKING_TIME)
        await sleep(5*1000)

        // Test queries
        delegatorBob = await getDelegator(ADDRESS_BOB, ADDRESS_ALICE, 0);
        expect(delegatorBob).toEqual(undefined);

        delegatorCharlie = await getDelegator(ADDRESS_CHARLIE, ADDRESS_ALICE, 0);
        expect(delegatorCharlie).toEqual(undefined);
        ...
        expect(preBalanceCharlie.plus(charlieDelegation)).toEqual(postBalanceCharlie);

        // Check pool delegation entry
        const delegationPoolData = await getDelegationPoolData(ADDRESS_ALICE, 0)
        expect(delegationPoolData).toEqual({"current_rewards": "0", "delegator_count": "0", "id": "0", "latest_index_k": "0", "latest_index_was_undelegation": false, "staker": "", "total_delegation": "0"});
    })
